...
open class RatingView: UIView {
    ...
    func customInit() {
        ...
        if offImage == nil {
            offImage = UIImage(named: "star-outline", in: bundle, compatibleWith: self.traitCollection)
        }
        if onImage == nil {
            onImage = UIImage(named: "star", in: bundle, compatibleWith: self.traitCollection)
        }
        
        if halfImage == nil {
            halfImage = UIImage(named: "star-outline", in: bundle, compatibleWith: self.traitCollection)
        }
    ...
    func layoutStars() {
        if stars.count != 0, let offImage = stars.first?.image {
            ...
            for iv in stars {
                ...
                iv.frame = CGRect(x: 0, y: 0, width: self.bounds.size.height, height: self.bounds.size.height)
